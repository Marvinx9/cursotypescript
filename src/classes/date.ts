...
export async function funcaoDeTempoInsert(
  vagas: number,
  totalMinutosInicial: number,
  duracao: string,
  id_agenda: string,
  ...
  for (let i = 0; i < vagas; i++) {
    let hora_agendamento = String(
      Math.floor((Number(totalMinutosInicial) + Number(duracao) * i) / 60),
    );
